    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <CategorySection title="Extension Data">
        <TestCard
          ...
          lookFor={`With the extension installed, you should see:\n- Full CPU model name instead of just core count (ARM-based Chromebooks may only show core count and architecture due to a ChromeOS platform limitation)\n- Exact RAM in GB instead of a capped estimate\n- Storage capacity and type\n- On managed ChromeOS: serial number, asset ID, hostname, location\n- With enterprise policies: manufacturer, model, MAC address, IP address\n\nIf data is missing, check that the extension is force-installed (not manually installed) and that the device is enrolled in your Google Workspace domain. Note: force-installing via a Google Group is user-scoped and may not enable device-level attributes like serial number. Apply the policy at the device OU level for full access.`}
        />
      </CategorySection>
